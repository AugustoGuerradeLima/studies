#include "io.h"

int main ( void )
{
 // dados/resultados
        char wordtake[100];
        char word[100];
        int len= 0;
        int charpass=0;
        int num;

 // identificar
    IO_id("Exemplo0315 v.0.0.");



 // acoes
       /* strcpy( word, IO_readstring("Insira uma palavra:"));*/

        printf("Insira uma palavra:");
        scanf("%s", wordtake);

        strcpy(word, wordtake);

        len = strlen(word);

        for (charpass=0; charpass<len; charpass++ )
        {
              if((word[charpass]>='0' && word[charpass]<='9')||(word[charpass]>='a' && word[charpass]<='z')||(word[charpass]>='A' && word[charpass]<='Z'))
              {
                     //nao faz nada
              }
              else
              {
                     printf("%c", word[charpass]);
                     num++;
              }
                     
                
        }//fim for
        
        //escrever a quantidade de minusculas
        printf("\n%s: %d", "Numero de simbolos diferentes de letras ou digitos",num);

 // encerrar
         printf("\nApertar ENTER para terminar.\n");
         getchar( );   
         return (0);
}
/*
Exercicio 1 ( OK )
Exercicio 2 ( OK )
Exercicio 3 ( OK )
Exercicio 4 ( OK )
Exercicio 5 ( OK )
Exercicio 6 ( OK )
*/
