...
int main( )
{
    ...
    //identificar
    IO_id("Exemplo 0214 - Programa - v.0.0");

    //ler no teclado
    number = IO_readint("Entre com um valor inteiro:");

    //tarefas
    if(15<=number && number<=45)
    {
        IO_printf("%s(%d)\n", "O valor pertence ao intervalo fechado [15:45]", number);

    }
    else
    {
        IO_printf("%s(%d)\n", "O valor nao pertence ao intervalo fechado [15:45]", number);
