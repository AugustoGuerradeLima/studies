...
double denoseven( int x )
{

    int cont = 0; //contador da funcao
    double number = 6;//numero com que se comeca
    double result = 0; //resultado a se retornar

    while(cont < x)
    {
        number++;//numero sempre incrementa
        result = result + (1.0 / number);//result soma com o valor de number

        cont++;//cont incrementa

        printf("+ ( 1 / %d )", (int)number);

    }//fim enquanto

    //escreve o resultado
    printf("\nO valor do resultado da soma dos naturais a partir do 7 com a quantidade definida e de: %lf ", result);

    //valor a se retornar
    return(result);

}//fim int denoseven( int x )
...
do
{
    ...
    IO_println("10 - mostrar a soma de certa quantia  de numeros naturais a partir do 7 como denominadores");
